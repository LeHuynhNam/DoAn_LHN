...
go
sp_rename 'maloai', 'loainv', 'NHANVIEN';
...
(
	...
	masp int not null,
	gia float,
	constraint pk_gia primary key (ngaycapnhat,masp),
	constraint fk_gia_sp FOREIGN KEY(masp) REFERENCES sanpham(masp),
)
drop table gia
...
go
create proc prc_getLogin @usn varchar(20), @pwd varchar(100)
as
begin
	select loainv from TAIKHOAN tk, NHANVIEN nv where nv.id=tk.id and tendangnhap=@usn and matkhau=@pwd
	--select loainv from TAIKHOAN tk, NHANVIEN nv where nv.id=tk.id and tendangnhap='1' and matkhau='1962026656160185351301320480154111117132155'
end
exec prc_getLogin '1' , '1962026656160185351301320480154111117132155'
...
--sua nhan vien
go
create proc prc_changeNV @id int, @ten nvarchar(30), @ngaysinh date, @diachi nvarchar(50), @mabhxh char(12), @loainv char(10), @cccd char(12)
as
begin
...
go
create proc prc_showTT
as
begin
	select tt.matau, loaitau, hoten from TAUTHUYEN tt left join NHANVIEN nv 
	on tt.thuyentruong=nv.id 
end
select * from TAUTHUYEN
--exec prc_showTT 
--hien thi danh sach thuyen vien theo tau
go
create proc prc_showNVbyTT @matau char(10)
as
begin
	select id,loainv,hoten,cccd from NHANVIEN where matau=@matau
end	


--lay danh sach thuyen vien chua co thuyen
go
create proc prc_getNVFree
as
begin
	select id,loainv, hoten, cccd from NHANVIEN where  loainv='TV' and matau is null
end
...
as
begin
	select id,loainv, hoten, cccd from NHANVIEN where  loainv='TT' and matau is null
end
...
begin
	...
	update nhanvien set matau=@matau where id=@ttmoi
end
--xoa thuyen truong khoi tau
go
create proc prc_delTT @matau char(10)
as
begin
	--set ma tau thuyen cua thuyen truong cu ve null
	update nhanvien set matau=null where matau=@matau and loainv='TT'
	--set ma thuyen truong moi vao tau moi
	update TAUTHUYEN set thuyentruong=null where matau=@matau
end
...
--kiem tra tau ton tai hay khong
go
create proc prc_findTT @matau char(10)
as
begin
	select * from TAUTHUYEN where matau=@matau
end

--them tau
go
create proc prc_addTT @matau char(10), @loaitau nvarchar(20)
as
begin
	insert into TAUTHUYEN values (@matau,@loaitau,null)
end
--sua tau thuyen
go
create proc prc_editTT @matau char(10), @loaitau nvarchar(20)
as
begin
	update TAUTHUYEN
	set loaitau=@loaitau
...
--xoa thuyen vien khoi thuyen
go
create proc prc_delMaTauNV @id int
as
begin
	update NHANVIEN set matau=null where id=@id
end
select * from sanpham
--show san pham
go
create proc prc_showSP
as
begin
	select *from sanpham
end
--tim san pham
go
create proc prc_findSP @text nvarchar(20)
as
begin
	select * from sanpham where tensp like '%'+@text+'%'
end
--show gia san pham theo ma
go
create proc prc_showGiaSP @masp int
as
begin
	select gia, ngaycapnhat from gia where masp=@masp
	order by ngaycapnhat desc
end
--them san pham
go
create proc prc_addSP @tensp nvarchar(20), @dvt char(10)
as
begin
	insert into sanpham values (@tensp, @dvt,0)
end
--lay ma sp
go
create proc prc_getIDSP
as
begin
...
--them gia san pham
go
create proc prc_addGiaSP @masp int, @ngaycapnhat date, @gia float
as
begin
	insert into gia values (@ngaycapnhat,@masp,@gia)
end
--sua gia san pham
go
create proc prc_editGiaSP @masp int, @ngaycapnhat date, @gia float
as
begin
	update gia
	set gia=@gia
	where masp=@masp and ngaycapnhat=@ngaycapnhat
end
--kiem tra gia san pham da duoc cap nhat ngay hom do chua
go
create proc prc_checkGiaSP @masp int, @ngaycapnhat date
as
begin
	select count(*) from gia where masp=@masp and ngaycapnhat=@ngaycapnhat
end
--lay danh sach tai khoan
go
create proc prc_showTK
as
begin
	select nv.id, hoten, tendangnhap
	from TAIKHOAN tk, NHANVIEN nv
	where tk.id=nv.id
end
--lay danh sach nhan vien chua co tai khoan
go
create proc prc_showNVChuaTK
as
begin
	select id, hoten from NHANVIEN where id not in (select id from TAIKHOAN where id is not NULL)
end
--them tai khoan
go
create proc prc_addTK @manv int, @tendangnhap varchar(20), @matkhau varchar(20)
as
begin 
	insert into TAIKHOAN values (@tendangnhap,@matkhau,@manv)
end
--kiem tra ten dang nhap
go
create proc prc_checkTenTK @tendangnhap varchar(20)
as
begin
...
--doi mat khau
go
create proc prc_editMKTK @tendangnhap varchar(20), @matkhau varchar(20)
as
begin
	update TAIKHOAN
	set matkhau=@matkhau
...
--tim kiem tai khoan
go
create proc prc_findTK @text varchar(20)
as
begin
	select nv.id, hoten, tendangnhap
	from TAIKHOAN tk, NHANVIEN nv
	where tk.id=nv.id and tendangnhap like '%'+@text+'%'
end
--xoa tai khoan
go
create proc prc_delTK @tendn varchar(20)
as
begin
...
--show hoa don
go
create proc prc_showHD
as
begin
	select mahd, tenkh, thanhtien, ngaylap,tinhtrang from HOADON hd, khachhang kh
	where hd.makh=kh.makh
end
--show chi tiet hoa don
go
create proc prc_showCTHD @mahd int
as
begin
	select tensp, soluong, gia, thanhtien from CTHOADON ct, sanpham sp
	where ct.masp=sp.masp and mahd=@mahd
end
--sua hoa don
go
create proc prc_editHD @mahd int, @tinhtrang nvarchar(20)
as 
begin
	update HOADON set tinhtrang=@tinhtrang
	where mahd=@mahd
end

--trigger tu dong cap nhat gia khi them


--trg tu dong cap nhta thanh tien, 
go
create trigger tgr_setCTHD
on cthoadon
for insert
as
begin
	update CTHOADON
	set cthoadon.gia=(select top 1 gia.gia from gia, inserted
				where gia.masp=inserted.masp
				order by ngaycapnhat desc )
	where mahd=(select mahd from inserted) and masp=(select masp from inserted)
	update sanpham
	set sltong= sltong-(select soluong from inserted)
	where masp=(select masp from inserted)
	update CTHOADON
	set thanhtien=gia*soluong
	where mahd=(select mahd from inserted) and masp=(select masp from inserted)
	update HOADON
	set thanhtien=thanhtien+(select thanhtien from CTHOADON where mahd=(select mahd from inserted) and masp=(select masp from inserted))
	where mahd=(select mahd from inserted)
end
--trg tu dong khi xoa
go
create trigger trg_delCTHD
on cthoadon
for delete
as
begin
	update sanpham
	set sltong=sltong+(select soluong from deleted)
	where masp=(select masp from deleted)
	update HOADON
	set thanhtien= thanhtien-(select thanhtien from CTHOADON where mahd=(select mahd from deleted) and masp=(select masp from deleted))
	where mahd=(select mahd from deleted)
end
--trg tu dong khi them CT hai trinh
go 
create trigger trg_setCTHT
on cthaitrinh
for insert
as
begin
	update CTHAITRINH
	set cthaitrinh.gia =(select top 1 gia.gia from gia, inserted
				where gia.masp=inserted.masp
				order by ngaycapnhat desc )
	where mahaitrinh=(select mahaitrinh from inserted) and masp=(select masp from inserted)
	update sanpham
	set sltong=sltong+(select soluong from inserted)
	where masp=(select masp from inserted)
	update CTHAITRINH
	...
	where mahaitrinh=(select mahaitrinh from inserted) and masp=(select masp from inserted)
	update HAITRINH
	set tongsanluong= tongsanluong+(select thanhtien from CTHAITRINH where masp=(select masp from inserted) and mahaitrinh=(select mahaitrinh from inserted))
	where mahaitrinh=(select mahaitrinh from inserted)
	update HAITRINH
	set doanhthu=tongsanluong-dautieuthu*giadau
	where mahaitrinh=(select mahaitrinh from inserted)
end

go
ALTER PROCEDURE PROC_REPORT_HOADON
@MAHD VARCHAR(50) 
AS
BEGIN
	SELECT * 
	FROM HOADON hd,CTHOADON ct,khachhang kh,sanpham sp
...
--exec prc_showHT 
create proc prc_showHT 
as
begin
	select * from HAITRINH 
end

--exec  prc_addHT @matau , @ngaydi , @ngayve , @dautieuthu , @giadau

create proc  prc_addHT
@matau char(10) , @ngaydi datetime , @ngayve datetime, @dautieuthu float, @giadau float
as
begin
	insert into HAITRINH
	values
	(@matau,@ngaydi,@ngayve,0,0,@dautieuthu,@giadau)
end


--exec  prc_addCTHT @maht ,  @masp , @sl , @gia
create proc prc_addCTHT
 @maht int,  @masp int, @sl float, @gia float
 as 
 begin
	insert into CTHAITRINH
	values
 ...
 --exec  prc_getMaHT
 create proc prc_getMaHT
 as 
 begin
 select top 1 mahaitrinh from HAITRINH order by mahaitrinh DESC
 end


 --exec prc_findHT @text
 go
 create proc prc_findHT @text nvarchar(20)
 as 
 begin
	select * from HAITRINH where mahaitrinh like'%'+@text+'%'
									or matau like'%'+@text+'%'
									or ngaydi like '%'+@text+'%'
									or ngayve like '%'+@text+'%'
									or tongsanluong like '%'+@text+'%'
									or doanhthu like '%'+@text+'%'
									or dautieuthu like'%'+@text+'%'
									or giadau like '%'+@text+'%'
 end


 --exec prc_showCTHT @maht
 create proc prc_showCTHT
 @maht int
 as
 begin
	select *from CTHAITRINH	where mahaitrinh=@maht
 end


 --exec prc_findHD @text
 go
 alter proc prc_findHD 
 @text nvarchar(max)
 as
 begin
 select mahd, tenkh, thanhtien, ngaylap,tinhtrang from HOADON hd, khachhang kh
	...
	( tenkh like'%'+@text+'%'
	or ngaylap like'%'+@text+'%'
	or tinhtrang like '%'+@text+'%')
 end

 
 go
 create proc  prc_addHD 
 @makh int,  @ngaylap datetime, @tinhtrang nvarchar(20)
 as
 begin
	
	insert into HOADON
	values
	(@makh,0,@ngaylap,@tinhtrang)
 end
 exec prc_addHD null, '2021-08-20 00:00:00.000','DATHANHTOAN'
 select * from hoadon
 --exec  prc_getMaHD 
 create proc prc_getMaHD
 as 
 begin
 select top 1 mahd from HOADON order by mahd DESC
 end

 --exec  prc_addCTHD @mahd ,  @masp , @sl , @gia
 create proc prc_addCTHD @mahd  int,  @masp int, @sl float, @gia float
 as
 begin
	insert into CTHOADON
	values
 ...
 --tim khach hang
 go
 alter proc prc_findKH @text nvarchar(max)
 as 
 begin
	select *from khachhang 
	where (tenkh like'%'+@text+'%'
		or makh like'%'+@text+'%'
		or sdt like'%'+@text+'%'
		or masothue like'%'+@text+'%'
		or diachi like'%'+@text+'%' 
		or loaikh like'%'+@text+'%' )
 end
 update sanpham
 ...
 --begin khachhang
create proc  prc_addKH 
 @tenkh nvarchar(50),  @sdt char(11), @masothue char(20),@diachi nvarchar(50),@loaikh char(10)
 as
 begin
	insert into khachhang
	values
	(@tenkh,@sdt,@masothue,@diachi,@loaikh)
 end

create proc prc_deleteKH
@id int
as 
begin
...
create proc prc_changeKH
 @id int,@tenkh nvarchar(50),  @sdt char(11),
  @masothue char(20),@diachi nvarchar(50),@loaikh char(10)
as
begin
	update khachhang
	set tenkh=@tenkh , sdt=@sdt,masothue=@masothue , diachi=@diachi,loaikh=@loaikh
	where makh=@id
end

exec prc_changeKH 11 ,'test' , '123' , '123' , '123' , '123'

 go
 --end khach hang
create proc prc_getGiaSP @ma float
 as 
 begin
	select top 1 gia from [dbo].gia
	where masp=@ma
	order by ngaycapnhat DESC
 end
 ...
 --proc lay loai nv
 go
 create proc prc_getLoaiNV @ma int
 as begin
